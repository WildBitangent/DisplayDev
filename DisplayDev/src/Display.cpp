...
Display* Display::getInstance()
{
	if(!mInstance)
	{
		mInstance = (Display* )malloc(sizeof(Display(8, 5)));
		new (mInstance) Display(8, 5);
	}
...
void Display::setResolution(uint8_t width, uint8_t height)
{
	...
	mFrontBuffer = (uint8_t* )realloc(mFrontBuffer, (width * height * 8) * sizeof(uint8_t));
	mBackBuffer = (uint8_t* )realloc(mBackBuffer, (width * height * 8) * sizeof(uint8_t));
}
...
Display::Display(uint8_t width, uint8_t height)
	...
	, mCurrentRow(0)
{
	mFrontBuffer = (uint8_t* )malloc((width * height * 8) * sizeof(uint8_t));
	mBackBuffer = (uint8_t* )malloc((width * height * 8) * sizeof(uint8_t));
...
void Display::Draw(uint16_t offset, uint8_t* buffer)
{
